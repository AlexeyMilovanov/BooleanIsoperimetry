#!/usr/bin/env python3
"""Prepare/run sectionwise Lean proof implementation loops.

This is the Harper-stability analogue of the Kolmogorov logic loop:

    Gemini -> Codex -> Gemini -> Codex -> Aristotle packet
...
and also tries to close/reconstruct more `sorry`s, then Aristotle receives the
remaining leaf obligations.
"""
...
def collect_context(section: dict[str, Any]) -> str:
    paper = read_text(DEFAULT_STABILITY_LAB_ROOT / "paper" / "harper-stability.tex", 70000)
    fable_audit = read_text(ROOT / "reviews" / "20260708T-fable-semantic-audit" / "AUDIT.md", 30000)
    latest_review = latest_file(ROOT / "interface_part_reviews", "*/SUMMARY.md")
    review_text = read_text(latest_review, 30000) if latest_review else "[no six-part review summary]\n"
    status = read_text(ROOT / "STATUS.md", 30000)
    readme = read_text(ROOT / "README.md", 12000)
    interface_hash = read_text(ROOT / ".interface.sha256", 4000)
    prior_aristotle = read_text(ROOT / "proof_loop" / "PRIOR_ARISTOTLE_ARTIFACTS.md", 12000)
    plan = read_text(DEFAULT_STABILITY_LAB_ROOT / "writeup-lean" / "01-FORMALIZATION-PLAN.md", 40000)
    return f"""
# Repository Context

Root: `{ROOT}`

## Frozen-Interface Hash

```text
{interface_hash.strip()}
```
...
## Latest six-part semantic review
...
def scan_sorries_at(root: Path) -> str:
    cmd = [
        ...
        "rg -n '\\bsorry\\b|sorryAx' HarperStability || true; "
        "else grep -RInE '\\bsorry\\b|sorryAx' HarperStability || true; fi",
    ]
    try:
...
def latest_strategy_outputs(section_dir: Path, before_iteration: int) -> str:
    ...
    for name in ["06_codex.md", "05_gemini.md", "04_codex.md", "03_opus.md", "02_codex.md", "01_gemini.md"]:
        path = latest / name
        if path.exists():
            chunks.append(f"\n\n## {name}\n\n")
            chunks.append(read_text(path, 50000).strip())
...
def ordinary_prompt(
    ...
    role = {
        ...
        "03_opus": "Claude Opus, deep implementation agent",
        "04_codex": "Codex, verifier and implementation repairer",
        "05_gemini": "Gemini, second implementation/repair agent",
        "06_codex": "Codex, final verifier and implementation repairer",
    }[stage]
    stage_task = {
        "01_gemini": (
        ...
        "03_opus": (
            "Continue from Codex's reviewed worktree. You are the strongest "
            "prover in this pipeline: pick the hardest remaining `sorry` and "
            "attack it with a complete, careful Lean proof (not a sketch). "
            "Verify every constant-dependence direction (K before sigma/eps) "
            "before writing; if the statement itself is defective, say so "
            "explicitly with STATUS: INTERFACE_PROBLEM and a counterexample "
            "sketch."
        ),
        "04_codex": (
            "Review Opus's changes. Keep correct progress, reject or revert "
            "unsound edits, repair build/type errors, and then try to close or "
            "strictly reconstruct more `sorry`s yourself."
        ),
        "05_gemini": (
            "Continue from Codex's reviewed worktree. Repair any remaining issues "
            "and try another implementation pass on the same section."
        ),
        "06_codex": (
            "Final implementation pass for this iteration. Verify the worktree, "
            "repair what you can, and leave a clean Aristotle packet for any "
            "remaining leaf obligations."
        ),
    }[stage]
    return f"""
...
def strategy_prompt(
    ...
    role = {
        ...
        "03_opus": "Claude Opus strategy deep-reviewer (verify the mathematics, especially quantifier order and constant dependence)",
        "04_codex": "Codex strategy critic",
        "05_gemini": "Gemini strategy repairer",
        "06_codex": "Codex final strategy writer",
    }[stage]
    return f"""
...
def write_aristotle_packet(
    ...
    project_label = f"HSLean_{section['id']}_iter{iteration:03d}_{mode}"
    if mode == "strategy":
        prompt = f"""
...
- Prefer proving leaf lemmas exactly as stated by Codex in `04_codex.md`.
...
"""
    ...
    if kind == "claude":
        ...
        cmd = [
            ...
            "acceptEdits",
            "--allowedTools",
            "Read,Edit,Write,Grep,Glob,LS,Bash(lake build:*),Bash(lake env:*),Bash(grep:*),Bash(rg:*),Bash(ls:*),Bash(cat:*),Bash(sed:*)",
            "--model",
            hp.CLAUDE_MODEL,
            "--effort",
            hp.CLAUDE_EFFORT,
        ...
        return run_process_capture(name=agent_id, cmd=cmd, cwd=work_root, prompt=launcher_prompt, out_dir=out_dir, timeout=timeout * 2)
    if kind == "codex":
        cmd = [
            hp.CODEX_BIN,
            "exec",
...
def allowed_prefixes(section: dict[str, Any]) -> list[str]:
    sid = section["id"]
    if sid == "U1_s4_uniform":
        return ["HarperStability/Process/EffectiveS4U.lean"]
    if sid == "U2_s5s6_uniform":
        return ["HarperStability/Core/EffectiveS5U.lean", "HarperStability/Core/EffectiveS6U.lean"]
    if sid == "U3_s7r2_uniform":
        return ["HarperStability/Core/EffectiveS7U.lean", "HarperStability/Reductions/EffectiveR2U.lean"]
    if sid == "U4_assembly_uniform":
        return ["HarperStability/Reductions/EffectiveR1U.lean", "HarperStability/Assembly/EffectiveUniform.lean"]
    if sid == "P1_geometry_volume":
        return ["HarperStability/Volume.lean", "HarperStability/Volume/"]
    if sid == "P2_entropy_probability":
        return ["HarperStability/Entropy.lean", "HarperStability/Entropy/"]
    if sid == "P3_reductions":
        return ["HarperStability/Reductions.lean", "HarperStability/Reductions/"]
    if sid == "P3a_reductions_r2_hbl":
        return ["HarperStability/Reductions/R2HBL.lean"]
    if sid == "P3b_reductions_r3_fiber":
        return ["HarperStability/Reductions/R3Fiber.lean"]
    if sid == "P4_process_s1_s4":
        return ["HarperStability/Process.lean", "HarperStability/Process/"]
    if sid == "P5a_core_s6_estimator":
        return ["HarperStability/Core/S6Estimator.lean"]
    if sid == "P5b_core_s6_envelope":
        return ["HarperStability/Core/S6.lean"]
    if sid == "P5c_core_s5_atom":
        return ["HarperStability/Core/S5.lean"]
    if sid == "P5c1_core_s5_kl":
        return ["HarperStability/Core/S5KL.lean"]
    if sid == "P5c2_core_s5_transfer":
        return ["HarperStability/Core/S5Transfer.lean"]
    if sid == "P5c3_core_s5_separation":
        return ["HarperStability/Core/S5Separation.lean"]
    if sid == "P5c4_core_s5_glue":
        return ["HarperStability/Core/S5.lean"]
    if sid == "P5a2_core_s6_window":
        return ["HarperStability/Core/S6Window.lean"]
    if sid == "E1_volume_eff":
        return ["HarperStability/Volume.lean", "HarperStability/Volume/"]
    if sid == "E2_r3_eff":
        return ["HarperStability/Reductions/EffectiveR3.lean"]
    if sid == "E3_s4_eff":
        return ["HarperStability/Process/EffectiveS4.lean"]
    if sid == "E4_s5_eff":
        return ["HarperStability/Core/EffectiveS5.lean"]
    if sid == "E5_s6_eff":
        return ["HarperStability/Core/EffectiveS6.lean"]
    if sid == "E6_s7_eff":
        return ["HarperStability/Core/EffectiveS7.lean"]
    if sid == "E7_r2_eff":
        return ["HarperStability/Reductions/EffectiveR2.lean"]
    if sid == "E8_r1a_eff":
        return ["HarperStability/Reductions/EffectiveR1.lean"]
    if sid == "E9_assembly_eff":
        return ["HarperStability/Assembly/Effective.lean"]
    if sid == "P5d_core_s7_heavy_ball":
        return ["HarperStability/Core/S7.lean"]
    if sid == "P5_core_s5_s7":
        return ["HarperStability/Core.lean", "HarperStability/Core/"]
    if sid == "P6_assembly":
        return ["HarperStability.lean", "HarperStability/Assembly.lean", "HarperStability/Assembly/"]
    return []


def is_allowed_candidate(rel: str, section: dict[str, Any]) -> bool:
    if rel.startswith("HarperStability/Interface/") or rel == "HarperStability/Interface.lean":
        return False
...
def safe_extract_project_tar(archive: Path, dest: Path) -> Path | None:
    ...
    candidates = sorted(dest.rglob("lakefile.lean"), key=lambda p: len(p.parts))
