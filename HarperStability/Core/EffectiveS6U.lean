...
lemma s6_eff_variance_grade5 (hR3 : R3Eff) (hS1 : S1Statement) (hS2 : S2Statement)
    (qMin qMax s0 mu0 : ℝ) (hqMin : 0 < qMin) (hqMinMax : qMin ≤ qMax) (hqMax : qMax < 1 / 2)
    (hs0 : 0 < s0) (hmu0 : 0 < mu0) (hreg : qMax + s0 ≤ 1 / 2 - mu0) :
    ∃ K_V : ℝ, 1 ≤ K_V ∧
      ∀ (sigma : ℕ → ℝ), Sublinear sigma → (∀ n : ℕ, 1 ≤ n → Real.log (n : ℝ) ≤ sigma n) →
      ∀ m (A : Finset (Cube m)) (q : ℝ),
        A.Nonempty → fat ⟨qMin, qMax, s0, mu0, sigma⟩ m A q → pinned ⟨qMin, qMax, s0, mu0, sigma⟩ m A q →
        ∃ vFam : ℝ → ℕ → ℝ,
          (∀ pLow, 0 < pLow → pLow ≤ 1 / 2 → varianceBudgetLE A pLow (vFam pLow m)) ∧
          (∀ pLow, 0 < pLow → pLow ≤ 1 / 2 →
            vFam pLow m ≤ (K_V / pLow) * (effEnv 5 sigma m + 1)) := by
  obtain ⟨K3, hK3, hR3_bound⟩ := hR3 qMin qMax s0 mu0 hqMin hqMinMax hqMax hs0 hmu0 hreg
  let K_V : ℝ := max (K3 * 100 + 100) 30
  have hKV : 1 ≤ K_V := le_max_of_le_right (by norm_num)
  refine ⟨K_V, hKV, ?_⟩
  intro sigma hsub hlog m A q hA hFat hPinned
...
lemma s6_eff_error_grade9 (qMin qMax s0 mu0 : ℝ) (hqMin : 0 < qMin) (hqMinMax : qMin ≤ qMax) (hqMax : qMax < 1 / 2)
    (hs0 : 0 < s0) (hmu0 : 0 < mu0) (hreg : qMax + s0 ≤ 1 / 2 - mu0)
    (K_V K_S5 : ℝ) (hKV : 1 ≤ K_V) (hKS5 : 1 ≤ K_S5) :
    ∃ K_err : ℝ, 1 ≤ K_err ∧
      ∀ (sigma : ℕ → ℝ), Sublinear sigma → (∀ n : ℕ, 1 ≤ n → Real.log (n : ℝ) ≤ sigma n) →
      ∀ m : ℕ, ∀ (vFam : ℝ → ℕ → ℝ) (bSlack : ℕ → ℝ),
        (∀ pLow, 0 < pLow → pLow ≤ 1 / 2 → vFam pLow m ≤ (K_V / pLow) * (effEnv 5 sigma m + 1)) →
        bSlack m ≤ (K_S5 / (min qMin s0 / 2) ^ 4) * (effEnv 8 sigma m + 1) →
        s6_errorBound (mu0 / 2) vFam bSlack (s6_eff_p ⟨qMin, qMax, s0, mu0, sigma⟩ m) (s6_eff_p ⟨qMin, qMax, s0, mu0, sigma⟩ m / 2) m ≤
          K_err * (effEnv 9 sigma m + 1) := by
  refine' ⟨ 40 * K_V / ( mu0 / 2 ) ^ 2 + 6 * K_S5 / ( min qMin s0 / 2 ) ^ 4 + 80 / ( mu0 / 2 ) ^ 2 + 1, _, _ ⟩;
  ...
  · intro sigma hsub hlog m vFam bSlack hvf hbs
    let Q : QData := ⟨qMin, qMax, s0, mu0, sigma⟩
    let hQ : validQData Q := ⟨hqMin, hqMinMax, hqMax, hs0, hmu0, hreg, hsub, hlog⟩
    set gap := Q.mu0 / 2
...
(`(0:ℝ)·H(…) = 0`). -/
lemma s6_eff_entropy_term_grade10 (qMin qMax s0 mu0 : ℝ) (hqMin : 0 < qMin) (hqMinMax : qMin ≤ qMax) (hqMax : qMax < 1 / 2)
    (hs0 : 0 < s0) (hmu0 : 0 < mu0) (hreg : qMax + s0 ≤ 1 / 2 - mu0)
    (K_err : ℝ) :
    ∃ K_H : ℝ, 1 ≤ K_H ∧
      ∀ (sigma : ℕ → ℝ), Sublinear sigma → (∀ n : ℕ, 1 ≤ n → Real.log (n : ℝ) ≤ sigma n) →
      ∀ m : ℕ, ∀ e : ℝ, 0 ≤ e →
        e ≤ K_err * (effEnv 9 sigma m + 1) →
        (m : ℝ) * H (min (e / (m : ℝ)) (1 / 2)) ≤ K_H * (effEnv 10 sigma m + 1) := by
  ...
  intro sigma hsub hlog m e he0 he_bound
  let Q : QData := ⟨qMin, qMax, s0, mu0, sigma⟩
  let hQ : validQData Q := ⟨hqMin, hqMinMax, hqMax, hs0, hmu0, hreg, hsub, hlog⟩
  have hsig : ∀ n, 0 ≤ Q.sigma n := fun n => hQ.2.2.2.2.2.2.1.1 n
...
lemma s6_eff_exp_remainder (qMin qMax s0 mu0 : ℝ) (hqMin : 0 < qMin) (hqMinMax : qMin ≤ qMax) (hqMax : qMax < 1 / 2)
    (hs0 : 0 < s0) (hmu0 : 0 < mu0) (hreg : qMax + s0 ≤ 1 / 2 - mu0) :
    ∃ C : ℝ, 0 ≤ C ∧
      ∀ (sigma : ℕ → ℝ), Sublinear sigma → (∀ n : ℕ, 1 ≤ n → Real.log (n : ℝ) ≤ sigma n) →
      ∀ m : ℕ, 2 * (m : ℝ) * Real.exp (-s6_eff_p ⟨qMin, qMax, s0, mu0, sigma⟩ m * (m : ℝ) / 8) ≤ C := by
  refine ⟨2048, by norm_num, ?_⟩
  intro sigma hsub hlog m
  let Q : QData := ⟨qMin, qMax, s0, mu0, sigma⟩
  let hQ : validQData Q := ⟨hqMin, hqMinMax, hqMax, hs0, hmu0, hreg, hsub, hlog⟩
  have hs : 0 ≤ effEnv 8 Q.sigma m :=
    effEnv_nonneg 8 Q.sigma (fun n => hQ.2.2.2.2.2.2.1.1 n) m
...
theorem s6_effU (hR3 : R3Eff) (_hS4U : S4EffU) (hS5U : S5EffU)
    (hS1 : S1Statement) (hS2 : S2Statement) (hS3 : S3Statement) : S6EffU := by
  intro qMin qMax s0 mu0 hqMin hqMinMax hqMax hs0 hmu0 hreg
  obtain ⟨K_V, hK_V, hvFam⟩ :=
    S6U.s6_eff_variance_grade5 hR3 hS1 hS2 qMin qMax s0 mu0
      hqMin hqMinMax hqMax hs0 hmu0 hreg
  obtain ⟨K_S5, hK_S5, hbSlack⟩ :=
    hS5U qMin qMax s0 mu0 hqMin hqMinMax hqMax hs0 hmu0 hreg
  obtain ⟨K_err, hK_err, herr_bound⟩ :=
    S6U.s6_eff_error_grade9 qMin qMax s0 mu0 hqMin hqMinMax hqMax hs0 hmu0 hreg
      K_V K_S5 hK_V hK_S5
  obtain ⟨K_H, hK_H, hH_bound⟩ :=
    S6U.s6_eff_entropy_term_grade10 qMin qMax s0 mu0 hqMin hqMinMax hqMax hs0 hmu0 hreg
      K_err
  obtain ⟨C, hC, hC_bound⟩ :=
    S6U.s6_eff_exp_remainder qMin qMax s0 mu0 hqMin hqMinMax hqMax hs0 hmu0 hreg
  ...
  let Q : QData := ⟨qMin, qMax, s0, mu0, sigma⟩
  let hQ : validQData Q := ⟨hqMin, hqMinMax, hqMax, hs0, hmu0, hreg, hsub, hlog⟩
  have h_vFam := hvFam sigma hsub hlog m A q hA hFat hPinned
  ...
  have hbSlack_bound := hbSlack sigma hsub hlog eps heps heps_lt m A q hA hFat hPinned
  let bSlack := fun m => (K_S5 / eps ^ 4) * (effEnv 8 Q.sigma m + 1)
  have h_bad : averageBadStepsLE A q eps (bSlack m) := hbSlack_bound
  let p := S6U.s6_eff_p Q m
  let pLow := p / 2
  have hp_bounds := S6U.s6_eff_p_bounds Q m hQ
  ...
  have h_apply := S6U.s6_eff_apply_s3 hS3 m A q Q.qMax eps gap hq_le hqMax heps hgap hgap_le vFam bSlack hA hvFam_bound1 h_bad hv_nonneg hb_nonneg p pLow hpLow_pos hpLow_le hp_bounds.2
  have h_center := uH_predictableCenter_le_coordField A
  have h_err_bound := herr_bound sigma hsub hlog m vFam bSlack hvFam_bound2 (le_refl _)
  have he_nonneg : 0 ≤ s6_errorBound gap vFam bSlack p pLow m := by
    ...
    linarith
  have hH := hH_bound sigma hsub hlog m (s6_errorBound gap vFam bSlack p pLow m) he_nonneg h_err_bound
  have h_term1 : 2 * Real.log 2 * p * (m : ℝ) ≤ (2 * Real.log 2) * (effEnv 9 Q.sigma m + 1) := by
    have h_pm : p * (m : ℝ) ≤ effEnv 9 Q.sigma m := S6U.s6_eff_p_mul_le_effEnv8 Q m hQ
  ...
  have h_term4 : 2 * (m : ℝ) * Real.exp (-p * (m : ℝ) / 8) ≤ C * (effEnv 9 Q.sigma m + 1) := by
    have h_C := hC_bound sigma hsub hlog m
  ...
  have h8le9 : effEnv 9 Q.sigma m + 1 ≤ effEnv 10 Q.sigma m + 1 := by
    have := S6U.s6_env_mono_grade Q.sigma 9 m
    linarith
